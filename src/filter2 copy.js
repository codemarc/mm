...
export async function filterCommand(args, options, logger) {
  ...
  try {
    // load the config
    const config = u.load()
    verbose(`${Object.keys(config).length} accounts: [${Object.keys(config).join(", ")}]`)
    ...
    // offer a deep explanation of the scan command
    if (isAccount("explain")) {
      explain()
      return
    }
...
const explain = () => {
  ...
  info(chalk.blueBright("-------------------"))
  info(`The filter command is used to manage email rules and filters.
It can be used to list all filters for an account, 
create missing folders or delete deprecated filtered folders.

  * shows account index, name and filter count when no arguments given
  * lists all filters for an account when account name is specified
  ...
  * scans filter folders when --scan option is used:
    - checks flagged messages in each folder 
    - builds list of email addresses found
    - saves results to config file
    - can be limited to specific filter with filter name
  * categorizes filtered messages when --categorize option is used:
    - uses OpenAI to analyze and categorize messages
    - categories are dynamically generated based on content
    - results are saved to output file
